...
function normalizeExperienceLevel(raw) {
    ...
    // Hebrew first — Elbit posts in Hebrew, and the English patterns below
    // would grade every one of those titles null.
    if (/סטודנט|מתמחה|התמחות/.test(s)) return 'intern';
    if (/בכיר|ראש צוות|ראש תחום|ראש מדור|מנהל|מובילת|\bמוביל\b|סמנכ/.test(s)) return 'senior';
    if (/ג׳וניור|ג'וניור|זוטר|מתחיל/.test(s)) return 'entry';
