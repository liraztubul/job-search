...
async function loadMeta() {
  ...
  fillSelect($('f-company'), meta.companies.filter((c) => c.count > 0)
    .map((c) => ({ value: c.id, name: c.name, count: c.count })));
  ...
  fillSelect($('f-location'), meta.locations);
...
function jobCard(job) {
  ...
  const jobMeta = el('p', { className: 'job-meta' },
    ...
    job.externalId ? ` · משרה מס׳ ` : '',
    job.externalId ? el('span', { className: 'mono ltr', textContent: job.externalId }) : null);
